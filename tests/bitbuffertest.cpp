...
TEST(BitBufferTest, ShiftsCorrectly) {
	...
	bb1.shift(4);
	ASSERT_EQ(20, bb1.wordsUsed) << "Doesn't store correct amount of bits after shifting";

	uint8_t ret[2];
	memcpy(ret, bb1.buffer, sizeof(uint8_t) * 2);
	EXPECT_EQ(0x21, bb1.buffer[0]) << "The first returned word is incorrect";
	EXPECT_EQ(0x43, bb1.buffer[1]) << "The second returned word is incorrect";
}
